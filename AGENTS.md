...
- **Serverless Function** — apex → www redirect (`apps/apex-redirect/`); Edge Services can't take a bare apex, so the apex ALIAS points here and the function holds the apex's managed cert. Kept warm by a `scaleway_function_cron` self-ping every 5 min (free-tier; `min_scale` stays 0 — provisioned time bills outside the free tier)
