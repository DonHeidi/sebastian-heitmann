...
import { PixelRocker } from './pixel-rocker';
...
export function Footer({ footer }: FooterProps) {
  return (
    <footer className="mx-auto max-w-[90rem] border-t-2 border-primary p-6 md:py-10 md:px-20">
      <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-1 flex-col gap-1">
          <span className="flex items-center gap-2 font-mono text-[0.6875rem] text-muted-foreground">
            <AsteriskMark size={11} tone="ink" />
            {footer.copyright}
          </span>
          <span className="font-mono text-[0.6875rem] text-muted-foreground">{footer.madeWith}</span>
        </div>
        <div className="flex flex-1 justify-center gap-6">
          <a href={`${DEV}/privacy`} className="font-mono text-[0.6875rem] text-muted-foreground no-underline transition-colors hover:text-foreground">
            {footer.privacyLabel}
          </a>
          <a href={`${DEV}/imprint`} className="font-mono text-[0.6875rem] text-muted-foreground no-underline transition-colors hover:text-foreground">
            {footer.imprintLabel}
          </a>
          <a href={`${DEV}/#contact`} className="font-mono text-[0.6875rem] text-muted-foreground no-underline transition-colors hover:text-foreground">
            {footer.contactLabel}
          </a>
        </div>
