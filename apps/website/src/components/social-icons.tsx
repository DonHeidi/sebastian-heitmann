...
export function LinkedInIcon({ className, size = 16 }: { className?: string; size?: number }) {
  return (
    // Decorative: every call site wraps this in a link that carries its own
    // aria-label, so the svg must NOT expose role="img" (Lighthouse svg-img-alt
    // fails role="img" elements that lack their own accessible name).
